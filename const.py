...
import enum
from typing import NamedTuple, Optional, Tuple, Iterable, Union
import collections
import babel.core
import babel.lists 
...
def setlang(*langs: str) -> babel.core.Locale:
    ...
    if hasattr(setlang, "_locale") and not langs: return setlang._locale
    ...
    for lang in filter(bool,langs):
        try:
            ...
            logging.warning("Cannot parse language '{code}'".format(code=lang))
    ...
    try:
        ...
        logging.error("No {domain}.mo found in {dir}".format(
            domain=_DOMAIN, 
            dir=_LANG_DIR,
            ))
        _ = lambda x : x
    ...
    try:
        ...
        logging.warning("No locale found for '{code}'".format(code=lang))
        new_locale = babel.core.Locale(_SOURCE_LANGUAGE)  

    folderisnew = ((not hasattr(setlang, "_folder")) or 
                   (setlang._folder != folder))
    if (langs and list(langs)[0]) or folderisnew:  
        setlang._locale, setlang._folder = new_locale, folder
        logging.debug("{locale} {path}".format(
            path=path, 
...
class Category(enum.EnumMeta):
    ...
    def __contains__(self, item):
        ...
        return (isinstance(item, enum.Enum) and 
                hasattr(self, item.name) and 
                item.value==self[item.name].value) 
    ...
    def __repr__(self):
        return "<category {name}>".format(name=self.__name__)
...
class Categorized(enum.Enum, metaclass=Category):
    ...
    def __getattr__(self, name):
        if (name == "_value_" or 
            not hasattr(self, "_value_") or 
            not hasattr(self._value_, name)
            ):
    ...
    def __setattr__(self, name, new_value):
        if (name == "_value_" or 
            ...
            raise AttributeError("Can't change attribute (name: {name}) "
                "of {enum}".format(name=name, enum=repr(self)))
            
    def __delattr__(self, name):
        if hasattr(self, "_value_") and hasattr(self._value_, name):
            raise AttributeError("Can't delete attribute (name: {name}) "
                "of {enum}".format(name=name, enum=repr(self)))
        else:
...
class Command(Categorized):
    ...
    def __eq__(self: "Command", other) -> bool:
        return (other.lower() == self.KEY if type(other) is str
...
class BoardOption(Categorized):
    ...
    def hash_board(
        ...
        ax.grid(True)
        ax.xaxis.set_ticks(np.arange(.5, cols +.5, 1))
        ax.yaxis.set_ticks(np.arange(.5, rows + .5, 1))
        plt.xlim(cols + .5, .5)
        plt.ylim(.5, rows + .5)
        return ax
...
class Directions(Categorized):
    ...
    def diagonal(self, dimensions):
        ...
        return tuple([x for x in self.any_direction(dimensions) 
                      if tuple(x) not in orthogonals])
...
class Game(NamedTuple):
    ...
    class PieceRules(NamedTuple):
        ...
        def RESERVES_STR(self: "PieceRules") -> str:  
            ...
            for index in range(len(self.INITIAL_RESERVES)):
                ...
                by_color.append(_("{number} {color}").format(
    ...
    def STRS(self: "Game") -> Tuple[str, ...]:  
        ...
        lines.append(_("Played on {board} {dimensions}").format(
        ...
        for index in range(len(self.PIECES)):
            ...
            lines.append(_("{shape}: {rules}").format(
...
class Move(Categorized):
    ...
    class Placement(NamedTuple):
        ...
        def __str__(self: "Placement") -> str:
            ...
            return _("{color} {shape} to {destination}").format(
                color = self.COLOR,
                shape = self.MARKER,
                destination = self.TO).capitalize()
